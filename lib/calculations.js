...
export function calculateItem(item, settings, targetWeeks = 6) {
  ...
  if (isSpirit && bottleML && nipML && nipML > 0) {
    nipsPerBottle = +(bottleML / nipML).toFixed(1)
    weeklyAvgOrdering = +(weeklyAvgUnits / nipsPerBottle).toFixed(2)
  }

  const targetStock = Math.ceil(weeklyAvgOrdering * targetWeeks)
  const currentStock = isSpirit && nipsPerBottle
    ? +(item.onHand / nipsPerBottle).toFixed(1)
    : item.onHand

  const unitsNeeded = Math.max(0, targetStock - currentStock)
  const orderQty = unitsNeeded === 0 ? 0 : Math.ceil(unitsNeeded / pack) * pack

  let priority = 'OK'
  if (orderQty > 0) {
    priority = currentStock <= weeklyAvgOrdering * 2 ? 'CRITICAL' : 'LOW'
  }
  ...
  return {
    ...
    unitsNeeded,
    orderQty,
    bottlesToOrder: isSpirit && nipsPerBottle && orderQty > 0
      ? Math.ceil(orderQty / nipsPerBottle)
      : null,
    priority
  }
}
