  ...
  do {
    ...
    for (const obj of data.objects || []) {
      if (obj.type === 'ITEM') {
        ...
        const costAmt   = d.item_cost?.amount      // cents
        variations[obj.id] = {
          itemId: d.item_id || '',
          variationName: d.name || 'Regular',
          sku: d.sku || '',
          sellPrice: priceAmt !== undefined ? +(priceAmt / 100).toFixed(2) : null,
          buyPrice:  costAmt  !== undefined ? +(costAmt  / 100).toFixed(2) : null,
        }
      }
    }
    cursor = data.cursor
...
export async function fetchSquareData(token, daysBack = 90) {
  ...
  for (const [name, vars] of Object.entries(itemGroups)) {
    ...
    // Use the Regular variation price, fall back to first variation
    ...
    items.push({
      ...
      squareBuyPrice:  priceVar?.buyPrice  ?? null,
    })
  }
